...
let selected = 0, baseHz = 660, audioContext, analyser, stream, raf, samples = [], attemptActive = false, pitchHistory = [], calibrating = false;
...
function autoCorrelate(buffer, sampleRate) {
  ...
  let start = 0, end = buffer.length - 1, threshold = 0.18;
  ...
  const trimmed = buffer.slice(start, end), size = trimmed.length, corr = new Float32Array(size);
  for (let lag = 0; lag < size; lag++) for (let i = 0; i < size - lag; i++) corr[lag] += trimmed[i] * trimmed[i + lag];
  let dip = 0; while (dip + 1 < size && corr[dip] > corr[dip + 1]) dip++;
  let peak = dip, best = -Infinity; for (let i = dip; i < size; i++) if (corr[i] > best) { best = corr[i]; peak = i; }
  // 소금은 숨결과 배음이 섞여 기본음이 약해질 수 있어, 너무 짧은 배음 주기는 제외합니다.
  if (!peak || peak < 12) return -1;
  const x1 = corr[peak - 1] || 0, x2 = corr[peak], x3 = corr[peak + 1] || 0, shift = (x3 - x1) / (2 * (2 * x2 - x1 - x3));
...
function listen() {
  ...
  if (hz > 140 && hz < 1600 && energy > .012 && energy < .94) {
    // 이전보다 약 10배 긴 시간(약 1초)의 소리를 모아 판단합니다.
    // 순간적인 입김, 떨림, 배음은 화면의 음정과 점수에 거의 반영되지 않습니다.
    pitchHistory.push(hz); if (pitchHistory.length > 110) pitchHistory.shift();
    if (pitchHistory.length >= 60) {
      ...
      if (wobble < 35) { updateLive(stableHz); if (calibrating) finishCalibration(stableHz); }
  ...
  } else if (energy < .012) { pitchHistory = []; }
...
async function start() {
  ...
  try { stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: false, noiseSuppression: false, autoGainControl: false } }); audioContext = new (window.AudioContext || window.webkitAudioContext)(); analyser = audioContext.createAnalyser(); analyser.fftSize = 2048; analyser.smoothingTimeConstant = .15; audioContext.createMediaStreamSource(stream).connect(analyser); $('startButton').disabled = true; $('stopButton').disabled = false; $('status').textContent = '마이크가 켜졌어요. 소리를 1초 이상 고르게 유지하면 안정된 음만 평가해요.'; attemptActive = true; samples = []; resetLive(); listen(); } catch (e) { $('status').textContent = '마이크 사용을 허용해 주세요. 학교 기기 설정에서 마이크 권한도 확인해 주세요.'; }
}
...
function playReference() {
  const c = new (window.AudioContext || window.webkitAudioContext)(), now = c.currentTime, duration = 1.35;
